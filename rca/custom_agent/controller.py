...
from rca.custom_agent.executor import execute_act
...
def control_loop(objective: str, plan: str, ap, bp, logger, max_step=15, max_turn=3):
    prompt = [
        {
            "role": "system",
    ...
    for step in range(max_step):
        note = [
            {
                "role": "user",
                "content": f"Continue your reasoning process for the target issue:\n\n{objective}\n\nFollow the rules during issue solving:\n\n{ap.rules}.\n\nResponse format:\n\n{format}",
            }
        ]
        attempt_actor = []
        try:
            ...
            if completed == "True":
                ...
                prompt.append(
                    {
                        "role": "user",
                        "content": summary.format(objective=objective, cand=bp.cand),
                    }
                )
            ...
            if not status:
                logger.warning("Self-Correction failed.")
    ...
    final_prompt = {
        "role": "user",
        "content": summary.format(objective=objective, cand=bp.cand).replace(
