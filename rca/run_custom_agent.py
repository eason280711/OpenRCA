...
def main(args, uid, dataset):
    ...
    eval_file = f"test/result/{dataset}/custom-{args.tag}-{configs['MODEL'].split('/')[-1]}.csv"
    obs_path = f"test/monitor/{dataset}/custom-{args.tag}-{configs['MODEL'].split('/')[-1]}"
    ...
    for folder in ("history", "trajectory", "prompt"):
        os.makedirs(f"{unique_obs_path}/{folder}", exist_ok=True)

    if not os.path.exists(eval_file):
        os.makedirs(f"test/result/{dataset}", exist_ok=True)
        eval_df = pd.DataFrame(
            columns=["instruction", "prediction", "groundtruth", "passed", "failed", "score"]
        )
    else:
        ...
        for i in range(args.sample_num):
            ...
            try:
                ...
                for step in result.trajectory:
                    nb.cells.append(nbf.new_markdown_cell(json.dumps(step, ensure_ascii=False)))
                ...
                with open(promptfile, "w", encoding="utf-8") as f:
                    json.dump({"messages": result.prompt}, f, ensure_ascii=False, indent=4)
                ...
                passed_criteria, failed_criteria, score = evaluate(
                    result.prediction, scoring_points
                )
